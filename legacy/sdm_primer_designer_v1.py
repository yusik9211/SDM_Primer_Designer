...
def best_ecoli_codon(aa):
    """Highest-frequency E. coli K-12 codon for amino acid (1-letter code)."""
...
def aa_mutation_to_dna(template, aa_pos, orig_aa, new_aa, cds_start=1):
    """
    Convert an amino acid substitution to DNA-level parameters.

    aa_pos    : 1-based amino acid position
    orig_aa   : expected original amino acid (1-letter)
    new_aa    : target amino acid (1-letter)
    cds_start : 1-based nt position of the first codon in the template (default 1)

    Returns (nt_pos, orig_codon, new_codon, info_dict)
    """
    ...
    nt_pos = cds_start + (aa_pos - 1) * 3      # 1-based nt start of codon
    cs = nt_pos - 1                              # 0-based
    ce = cs + 3
...
def count_mm(orig, new):
    """Substitution count + indel length difference."""
...
def _primer_candidates(tmpl, ms, me, mut, n_mm, fwd, max_len=70):
    """
    Collect ALL valid single-primer candidates (satisfying G/C-end and Tm >= 78).
    Returns list of dicts, sorted by primer length ascending.

    fwd=True  (sense):    5'-[fl5>=4 upstream]-[MUT]-[fl3>=8 downstream]-3'
    fwd=False (antisense): 5'-[RC(fl5>=4 downstream)]-[RC(MUT)]-[RC(fl3>=8 upstream)]-3'
    """
    ML5, ML3 = 4, 8
    results = []
...
def find_primers(tmpl, ms, me, mut, n_mm, max_len=70):
    """
    Find the shortest PAIR (fwd, rev) satisfying all SDM rules, including
    the non-overlapping 3'-end constraint enforced jointly:

        fl3_fwd >= fl5_rev + 8   ->  fwd 3'-end extends >=8 nt beyond rev body
        fl3_rev >= fl5_fwd + 8   ->  rev 3'-end extends >=8 nt beyond fwd body

    This guarantees each primer has >=8 bases at its 3'-end that do NOT overlap
    with the body of the other primer.
    """
    ...
    for f in fwd_list:
        # Early exit: even with the shortest rev primer, total > best
        if f["len"] + (rev_list[0]["len"] if rev_list else 0) >= best_total:
            continue
        for r in rev_list:
            if f["len"] + r["len"] >= best_total:
                break  # rev_list is sorted; no shorter pair possible here
            # -- non-overlapping condition --------------------------
            # Forward's 3'-end non-overlapping portion = fl3_fwd - fl5_rev >= 8
            if f["fl3"] < r["fl5"] + 8:
                continue
            # Reverse's 3'-end non-overlapping portion = fl3_rev - fl5_fwd >= 8
            if r["fl3"] < f["fl5"] + 8:
                continue
...
def design(template, pos, orig, mut):
    """
    Design forward + reverse SDM primers.

    pos  : 1-based mutation start (nt)
    orig : original sequence ('' for insertion)
    mut  : mutated sequence ('' for deletion)
    """
    T = validate_dna(template)
    ...
    # Non-overlapping counts (for display/validation)
...
def _annotate_fwd(seq, muts_sorted, primer_start):
    """
    Annotate forward primer: lowercase = template-match, [UPPER] = mutated.
    muts_sorted : [(ms, me, orig, new), ...] in ascending order, 0-based
    primer_start: 0-based template position of the primer's first base
    """
    result = []
    seq_i = 0
...
def _annotate_rev(seq, muts_sorted, fl5_rev):
    """
    Annotate reverse (antisense) primer.
    RC primer structure (5'->3'):
      [RC(downstream fl5)] [RC(mut_last)] ... [RC(mut_1)] [RC(upstream fl3)]
    fl5_rev: number of downstream template bases at 5' end of rev primer
    """
    result = []
    seq_i = 0
    n = len(muts_sorted)
...
def design_combined(template, mutations):
    """
    Design ONE forward+reverse primer pair carrying ALL mutations at once.

    mutations : list of (pos_1based, orig_str, new_str)
                orig_str = '' for pure insertion
                new_str  = '' for pure deletion

    Returns a result dict compatible with show_combined(), or raises ValueError.
    """
    T = validate_dna(template)
...
def show_combined(r, label="COMBINED PRIMER DESIGN", codon_infos=None):
    W = 68
    sep  = "=" * W
    thin = "-" * W
    muts = r["muts"]   # [(ms, me, orig, new), ...] 0-based sorted
    ...
    for key, label_p, fwd_flag in [
        ("fwd", "FORWARD 5'->3' (sense)", True),
        ("rev", "REVERSE 5'->3' (antisense)", False),
    ]:
        p = r[key]
        print()
        print(thin)
...
def show(r):
    ...
    # -- primers --
    for key, label in [
        ("fwd", "FORWARD PRIMER   5'->3'  (sense strand)"),
        ("rev", "REVERSE PRIMER   5'->3'  (antisense strand)"),
    ]:
        p = r[key]
        print()
        print(thin)
...
def _resolve_mutation(tmpl, raw, cds_start):
    """
    Parse one mutation line and return (pos_nt, orig, new, codon_info_or_None).

    Formats accepted:
      nt <pos> <orig> <new>    -- DNA level  (prefix 'nt' optional)
      aa <pos> <orig_aa> <new_aa>  -- AA level
      <pos> <orig> <new>       -- DNA level (no prefix)
    """
...
def run_batch(tmpl, cds_start):
    """
    Collect multiple mutations, then:
      1. Design individual primer pairs for each mutation.
      2. Design one combined primer pair covering all mutations (if > 1).
    """
    print()
    ...
    collected = []   # list of (pos, orig, new, codon_info)
    idx = 1
    while True:
    ...
    try:
        ...
        show_combined(rc, label="COMBINED PRIMER DESIGN", codon_infos=codon_infos)
    except ValueError as e:
        print()
...
def run_once(tmpl, cds_start):
    """Ask for one mutation (DNA or AA mode) and show results."""
...
def main():
    print()
    print("  +--------------------------------------------------+")
    print("  |   SDM Double Primer Designer                     |")
